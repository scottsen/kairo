...
class AgentToSCFPass:
    ...
    def _process_operation(self, op: Any) -> None:
        ...
        if AgentDialect.is_agent_op(op):
            ...
            if op_name == "kairo.agent.spawn":
                self._lower_agent_spawn(op)
            elif op_name == "kairo.agent.update":
                self._lower_agent_update(op)
            elif op_name == "kairo.agent.query":
                self._lower_agent_query(op)
            elif op_name == "kairo.agent.behavior":
...
def create_agent_to_scf_pass(context: KairoMLIRContext) -> AgentToSCFPass:
