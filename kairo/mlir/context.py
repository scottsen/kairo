"""MLIR Context Management for Kairo v0.7.0

This module provides the foundational MLIR context management for Kairo's
...
class KairoMLIRContext:
    """Manages MLIR context for Kairo compilation.

    This class provides a centralized context for all MLIR operations in Kairo,
    ...
    def _register_dialects(self):
        """Register standard and custom Kairo dialects.
    ...
    def create_module(self, name: Optional[str] = None) -> "ir.Module":
        ...
        Example:
            >>> module = ctx.create_module("my_kairo_program")
        """
        with self.ctx:
...
def get_mlir_context() -> KairoMLIRContext:
    """Get or create the global Kairo MLIR context.

    Returns:
        Shared KairoMLIRContext instance
    ...
    if _global_context is None:
        _global_context = KairoMLIRContext()
...
_global_context: Optional[KairoMLIRContext] = None
