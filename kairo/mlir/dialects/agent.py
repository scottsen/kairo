...
class AgentType:
    ...
    def get(element_type: Any, context: Any) -> Any:
        ...
        return ir.OpaqueType.get("kairo", f"agent<{element_str}>", context=context)
    ...
    def create(
        ...
        with loc, ip:
            ...
            result.operation.attributes["op_name"] = ir.StringAttr.get(
                "kairo.agent.spawn", context=loc.context
            )
...
class AgentUpdateOp:
    ...
    def create(
        ...
        with loc, ip:
            ...
            # Add attributes
            result.operation.attributes["op_name"] = ir.StringAttr.get(
                "kairo.agent.update", context=loc.context
            )
...
class AgentQueryOp:
    ...
    def create(
        ...
        with loc, ip:
            ...
            # Add attributes
            result.operation.attributes["op_name"] = ir.StringAttr.get(
                "kairo.agent.query", context=loc.context
            )
...
class AgentBehaviorOp:
    ...
    def create(
        ...
        with loc, ip:
            ...
            # Add attributes
            result.operation.attributes["op_name"] = ir.StringAttr.get(
                "kairo.agent.behavior", context=loc.context
            )
...
class AgentDialect:
    ...
    def get_agent_op_name(op: Any) -> Optional[str]:
        ...
        Returns:
            Operation name (e.g., "kairo.agent.spawn") or None
        """
        if not MLIR_AVAILABLE:
            return None
