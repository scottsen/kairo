...
**Related:** ADR-002 (Cross-Domain Patterns), DOMAIN_ARCHITECTURE.md
...
- ✅ Read `DOMAIN_ARCHITECTURE.md` (understand domain classification)
...
- ✅ Familiarize yourself with Kairo's type system (`docs/SPEC-TYPE-SYSTEM.md`)
...
BodyRef
  ...
  - Frame: Spatial frame (inherits from SPEC-COORDINATE-FRAMES.md)
```
...
docs/
├── SPEC-PHYSICS.md                   # Domain specification
├── ADR/
...
1. **docs/SPEC-PHYSICS.md** - Specification first (design before code)
2. **kairo/mlir/dialects/physics.py** - MLIR dialect (types + operations)
3. **kairo/stdlib/physics.py** - Python API (user-facing operators)
4. **kairo/mlir/lowering/physics_to_scf.py** - Lowering to MLIR
5. **kairo/mlir/passes/physics/*.py** - Optimization passes
...
**File:** `docs/SPEC-{DOMAIN}.md`

**Template:**
```markdown
# SPEC-{DOMAIN}: {Full Domain Name}
...
**See:** `docs/SPEC-GEOMETRY.md` as a reference implementation.

---

### Step 2: Define MLIR Dialect

**File:** `kairo/mlir/dialects/{domain}.py`
...
**Reference:** See `kairo/mlir/dialects/audio.py` for a complete example.

---

### Step 3: Implement Python API (stdlib)

**File:** `kairo/stdlib/{domain}.py`
...
**Reference:** See `kairo/stdlib/audio.py` for Layer 3-4 operators.

---

### Step 4: Implement Lowering to MLIR

**File:** `kairo/mlir/lowering/{domain}_to_scf.py`
...
**Reference:** See `kairo/mlir/lowering/audio_to_scf.py`.

---

### Step 5: Implement Domain Passes

**File:** `kairo/mlir/passes/{domain}/{pass_name}.py`
...
**Reference:** See `kairo/mlir/passes/` for existing passes.
...
1. **Specification** (`docs/SPEC-{DOMAIN}.md`)
...
- [ ] README in `kairo/stdlib/{domain}/` explaining domain purpose
- [ ] Entry in `docs/DOMAIN_ARCHITECTURE.md` (update roadmap)
...
- [ ] Type checking passes (`mypy kairo/stdlib/{domain}.py`)
- [ ] Linting passes (`ruff check kairo/stdlib/{domain}.py`)
...
4. **`docs/SPEC-OPERATOR-REGISTRY.md`**
...
1. **Define interface contract** in `kairo/interfaces/{source}_to_{target}.py`
   ```python
...
1. **CUDA lowering pass**: `kairo/mlir/passes/{domain}/cuda_lowering.py`
