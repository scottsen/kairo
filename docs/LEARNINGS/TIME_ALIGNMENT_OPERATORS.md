...
**Domain:** AudioDomain
**Related:** SPEC-OPERATOR-REGISTRY.md, AUDIO_SPECIFICATION.md, OPERATOR_REGISTRY_EXPANSION.md
...
These operators extend the existing Transform layer (Layer 2) from `SPEC-OPERATOR-REGISTRY.md`.
...
### Already Exists (From SPEC-OPERATOR-REGISTRY.md)
...
These operators fit into the existing **7-layer operator architecture** from `SPEC-OPERATOR-REGISTRY.md`:
...
Time alignment fits into Kairo's domain architecture (from `DOMAIN_ARCHITECTURE.md`):
...
Following the pass architecture from `SPEC-OPERATOR-REGISTRY.md`:
...
- **SPEC-OPERATOR-REGISTRY.md** — Operator registry structure (7 layers)
...
- **SPEC-TRANSFORM.md** — Transform operators (FFT, STFT)
