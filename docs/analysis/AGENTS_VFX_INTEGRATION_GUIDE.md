...
|------|------|---------|-------------------|
| `kairo/stdlib/agents.py` | 544 LOC | Core agent system | Add new operations (emit, lifetime, collision) |
| `kairo/mlir/dialects/agent.py` | 526 LOC | High-level ops | Add behavior types (fade, explode, vortex) |
| `kairo/mlir/lowering/agent_to_scf.py` | 424 LOC | Lowering to loops | Extend behavior implementation |
| `kairo/stdlib/visual.py` | 781 LOC | Rendering | Add `particles()` and `trails()` functions |
...
- [ ] **Behavior Templates** - Extended behavior types
  - File: `kairo/mlir/lowering/agent_to_scf.py` lines 322-394
  - Add: fade, explode, vortex, damping behaviors
  
- [ ] **Particle Pools** - Efficient allocation reuse
  - File: `kairo/stdlib/agents.py` new class
