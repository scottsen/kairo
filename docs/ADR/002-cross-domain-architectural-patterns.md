...
1. Scan `kairo/stdlib/{domain}/` (built-in operators)
2. Scan `~/.kairo/domains/{domain}/ops/` (user operators)
...
**Implementation:**
```python
# kairo/mlir/passes/physics/symplectic_enforcement.py
...
```python
# kairo/interfaces/cross_domain.py
...
- **DOMAIN_ARCHITECTURE.md:** Comprehensive domain vision
- **SPEC-OPERATOR-REGISTRY.md:** Operator metadata schema
- **SPEC-COORDINATE-FRAMES.md:** Frames and anchor system
...
| **Extensibility** | `~/.tiacad/ops` | `~/.riffstack/ops` | Pattern combinators | `~/.kairo/domains/{name}/ops` |
